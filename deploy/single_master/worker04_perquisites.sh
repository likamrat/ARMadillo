...
# Change the hostname
sudo hostnamectl --transient set-hostname $WORKER04_HOSTNAME
sudo hostnamectl --static set-hostname $WORKER04_HOSTNAME
sudo hostnamectl --pretty set-hostname $WORKER04_HOSTNAME
sudo sed -i s/raspberrypi/$WORKER04_HOSTNAME/g /etc/hosts

# Set the static ip
sudo cat <<EOT >> /etc/dhcpcd.conf
interface eth0
static ip_address=$WORKER04_IP/24
static routers=$DNS
