...
sudo hostnamectl --static set-hostname $WORKER05_HOSTNAME
sudo hostnamectl --pretty set-hostname $WORKER05_HOSTNAME
sudo sed -i s/raspberrypi/$WORKER05_HOSTNAME/g /etc/hosts

# Set the static ip
sudo cat <<EOT >> /etc/dhcpcd.conf
interface eth0
static ip_address=$WORKER05_IP/24
static routers=$DNS
