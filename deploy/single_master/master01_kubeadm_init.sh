...
# Getting status
echo "Waiting for the Master node to be in 'Ready' state (sleeping 150s)"
sleep 150
...
for host in ${WORKERS_HOSTS}; do
    ...
    sudo sshpass -p $Pi_PASSWORD ssh -o StrictHostKeyChecking=no $Pi_USERNAME@$host 'source ARMadillo/deploy/single_master/env_vars.sh'
